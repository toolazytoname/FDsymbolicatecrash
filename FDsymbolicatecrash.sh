...
#
# 注意事项：(1) 脚本用到了wget，需提前安装好（也用到了unzip，不过这个应该是系统默认已经装好了）。
...
#         (6) symbolicatecrash 的文件我写死成 /Applications/Xcode.app/Contents/SharedFrameworks/DVTFoundation.framework/Versions/A/Resources/symbolicatecrash
...
#             执行 ./FDsymbolicatecrash.sh 会根据本地已经有的dSYM文件进行解析。
#--------------------------------------------
